...
app.get('/users', async (req, res) => {
    const result = await conn.query('SELECT * FROM uesrs');
    res.json(result[0])
});
...
app.post('/users', async (req, res) => {
    try {
        let user = req.body;
        const results = await conn.query('INSERT INTO uesrs Set ?', user);
        res.json({
...
app.get('/users/:id', async (req, res) => {
    try {
        ...
        const results = await conn.query('SELECT * FROM uesrs WHERE id = ?', id);
...
app.put('/users/:id', async (req, res) => {
    try {
        ...
        const results = await conn.query('UPDATE uesrs SET ? WHERE id = ?', [updatedUser, id]);
        res.json({
...
app.delete('/users/:id', async (req, res) => {
    try {
        let id = req.params.id;
        const results = await conn.query('DELETE FROM uesrs WHERE id = ?', id);
        res.json({
